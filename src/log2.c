...
double log2(double x)
{
    ...
    if((0x000fffff&(2+hx))<3) {     /* |f| < 2**-20 */
        ...
        if(k==0) return f-R; else {dk=(double)k; return dk*ln2_hi-((f-R)-dk*ln2_lo);}
    }
    ...
    if(i>0) {
        hfsq=0.5*f*f;
        if(k==0) return f-(hfsq-s*(hfsq+R)); else return dk*ln2_hi-((hfsq-(s*(hfsq+R)+f))-dk*ln2_lo);
    } else {
        if(k==0) return f-s*(f-R); else return dk*ln2_hi-((f-s*(f-R))-dk*ln2_lo);
    }
}
